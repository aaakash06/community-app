...
const TagDetails = async ({
  ...
  const tag = await getTagById(params.id, searchParams.q, searchParams.filter);

  return (
    <>
      <h1 className="h1-bold text-dark100_light900 max-sm:text-[20px] mb-10">
        {tag.name}
      </h1>

      <div className="mid flex gap-4  flex-col max-md:flex-row max-sm:flex-col">
        <SearchBar route={`/tags/${params.id}`} />
